        ...
        chats.forEach(chat => {
          ...
          for (const iterator of chat.participantsMeta) {
            if (iterator.idUser != user.idUser) {
              ...
              chat.unreadMessagesLocal = chat['unreadMessage_' + user.idUser] ? chat['unreadMessage_' + user.idUser] : null;
  
            }
          }
  
          if(chat['unreadMessage_' + user.idUser] && chat['unreadMessage_' + user.idUser] > 0){
    ...
    */
    ...
    async createChatWithSupport(userCreated: iUser) : Promise<any> {
      ...
      try {
        ...
        console.log(error);
        this.utilService.showAlert('Info', 'Error Creating chat With Support. Please try again Later');
        return;
      }
      ...
      try {
        ...
        if (chatRef.empty ) { // Create NEW CHAT
          ...
          return new Promise((resolve) => {
            ...
            this.utilService.showAlert('Information' , 'Already exists a chat created with the support');
    ...
    async pushNewMessageChat(idChat: string, message: iMessage, idUserReciever: string) {
      ...
      try {
        ...
        const increment = this.utilService.getInstanceFirebase().firestore.FieldValue.increment(1);
    ...
    async clearUnreadMessagesFirebase(idChat: string, idUser : string , decrementReadedMessages : number) {
  
      const increment = this.utilService.getInstanceFirebase().firestore.FieldValue.increment(-decrementReadedMessages);
    ...
    async updateLastMessageReaded(idChat : string , idUserReciever : string , idMessage : string){
